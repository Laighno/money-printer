...
**★ 当前 production deterministic baseline：BlendRanker + Conviction + FACTOR_COLUMNS 64 + EXCESS_CAP winsorize + 全 deterministic chain (N=3 distribution)**
（2020-01 ~ 2026-04，PIT-clean，hs300+zz500 universe，2026-05-26 P10-1 N=3 BlendRanker A/B — 见下表 + decision_log "## P10 chain"）

| Metric | seed 42 (production-realized ★) | seed 43 | seed 44 (worst) | mean (N=3) | std |
|---|---:|---:|---:|---:|---:|
| **Sharpe** | **1.90** | 1.89 | 1.67 | **1.82** | 0.13 |
| annual_return | 60.42% | 60.32% | 51.66% | 57.47% | — |
| annual_volatility | 31.85% | 31.86% | 30.93% | 31.55% | — |
| max_drawdown | -36.30% | -33.31% | -38.16% | -35.92% | — |

**配置**: `RANKER_KIND=blend` (P10-2 default), `EXCESS_CAP=0.50` (winsorize ON), `WF_FEATURE_PRESET=W_BASELINE`, deterministic chain (`deterministic=True num_threads=1 force_row_wise=True`, `PYTHONHASHSEED=0`), `--skip-update` 保护 production. 详细 audit trail 见 `docs/decision_log.md` "## P10 chain · BlendRanker A/B winsorize lift confirmed".

**生产链对齐**: 该 baseline 用 `RANKER_KIND=blend` (P10-2 default, walk_forward_backtest.py:148), 与 `scripts/paper_trade.py` 加载的 single `BlendRanker` 一致 (Rule #11 满足). 之前 P7-γ 时代锁的 1.20 是 `RANKER_KIND=stock` 默认下 StockRanker 测量 — production 路径不一致, 已被 P10-1 N=3 A/B 数据 supersede.

数据源：N=3 BlendRanker A/B (P10-1, advisor round 66-68, engineer commit `925cc12`).
Verify run logs: `/tmp/wf_blend_{OLD,NEW}_seed{42,43,44}.log`. Run D byte-perfect deterministic verify (NEW seed 44 = 1.06 byte-identical to round 60 Run 2, post-P9-2 env fix) confirms chain integrity.
...
>
> **阈值统计无 grounding caveat** (P5-A-light, round 41)：以下黄/红阈值是
> **absolute pain levels**，不是 σ-grounded。`cross-seed σ = 0.13`（β0 spike）
> 测的是 seed lottery 不是 weekly time-series drift；weekly walk-forward σ 在
> 本仓库**从未测量过**。所以"RED Sharpe 0.9 = -7σ"这种 framing 是 type error。
> 实际语义：这是"catastrophic breach 不要 silent"backstop，**不是 statistically
> significant degradation 检测器**。Proper grounding 是 P5-A-mid 后续候选
> （~4-6 hr 跑 N=8-12 weeks 时间序列 σ 估算），未排期。详见
> [`mp/monitor/threshold_alert.py`](../../mp/monitor/threshold_alert.py) docstring。

| 指标 | BASELINE ★ | 黄色告警 | 红色告警（即停模拟交易）| 为什么 |
|---|---:|---:|---:|---|
| 最近 12 月滚动年化 | 60.42% | < 30% | < 15% 或 < 3× ZZ500 | 策略核心假设失效 |
| Sharpe（最近 252 日）| 1.82 (N=3 mean; 1.67 worst-seed) | < 1.0 | < 0.5 | 风险调整后已无明显优势 (P10-2: re-anchor based on N=3 worst-seed) |
