...
- **Round 160–161 (Phase 1 dryrun + user 3-detail OK)**: Backup created (`*.c2c_backup_20260530_2050`), n2c BlendRanker trained to `data/blend_n2c_DRYRUN_*.lgb` (primary IC 0.093 / extreme 0.103). Dryrun added c2c-FRESH control: n2c vs c2c-FRESH = 8/10 overlap (label-only effect small ✓); n2c vs prod c2c = 0/10 (training-time drift, not label, since prod c2c was May 24). User round 161 ack'd all 3 sub-questions: (i) only swap n2c → prod (no c2c refresh), (ii) StockRanker fallback retrain n2c, (iii) `data/model_60d.lgb` untouched.
- **Round 162 (Phase 2 — Step A swap + Step B 4 guardrails, 2026-05-31)**:
  - **Step A**: `data/blend_n2c_DRYRUN_*.lgb` → `data/blend_*.lgb` (cp; backups kept). StockRanker n2c retrain → `data/model.lgb` (IC=0.057, MAE=0.083 from cache-only path on `factors_label_next_open_to_close.parquet`). Sanity check 2026-03-30 top10 = 10/10 match round 160 dryrun list ✓. BASELINE.md updated.
  - **Step B (a) cap ≤20000元**: `mp/risk/arm_b_budget.py::ArmBBudgetTracker` (JSON-persisted, daily-keyed). Wired into `scripts/execute_orders.py::run()` via `arm_b_tracker=` arg; auto-engaged when `plan.entry_path == "intraday_14_30"`. Implementation at entry layer (not broker) — bypassable in theory but the production path is intraday_plan → orders.json → execute_orders → broker (no direct broker calls); broker-layer wrapper deferred unless advisor escalates. 7-step unit smoke + end-to-end smoke (3 buys, 1 should-block) both pass.
  - **Step B (b) liquidity + price**: ADV(20d)≥1亿 already in `apply_top_k_filters` (`LOW_LIQUIDITY_FILTER_AMOUNT`). Added `ARM_B_PRICE_CAP=50.0` + optional `price_map: Dict[str,float]` arg (fed by 14:29 morning-bar close). EOD path passes no `price_map` → behaviour unchanged. Smoke verified boundary (49.99 keep, 50.01 drop, 0/missing drop).
  - **Step B (c) report**: `scripts/oos_arm_b_report.py` → `data/reports/oos_arm_b_<YYYYMM>.md`. Cron-friendly (exit 0 always). Caveat documented inline: NAV not bucket-separated yet — report shows mixed NAV trajectory + bucket-level execution count/notional. Bucket-NAV separation is a follow-up.
  - **Step B (d) hard stop**: `mp/risk/freeze.py` (file flag `data/.real_money_frozen`, `guard_or_raise(mode)` API; dryrun bypasses) + `scripts/arm_b_stop_monitor.py` (cron; exit codes 0/1/2/3 for OK/warn/triggered/error). `execute_orders.py` checks `guard_or_raise(args.mode)` post argparse, returns 4 if blocked. Smoke: simulate → freeze flag written, alert emitted, `Disable-ScheduledTask` PowerShell line printed for ECS ops; execute_orders interactive→exit 4, dryrun→exit 0 (intentional bypass). `--reset` clears flag for debug. Threshold: hard ≤-5pp (per round 159), warn ≤-3pp (engineer-added yellow card).
  - 真钱 + 14:30 task: **仍 Disabled / 仍冻结**. Awaits user + advisor confirmation before Phase 3.
- **Round 163 (advisor ACK Phase 2 + user 拍板进 Phase 3, 2026-05-31)**: 5 review points all accepted by advisor; user approved 3-step Phase 3 plan (Step C dryrun → Step D unfreeze + ECS Enable → Step E ongoing monitor).
- **Round 164 (Step C final dryrun, 2026-05-31, commit 3b44851)**: `scripts/dryrun_phase3_picks.py` runs n2c BlendRanker on cached 2026-04-28 panel; top10 priced ¥2.60–¥15.93 (all under guardrail (b) cap), estimated total ¥17,566/¥20,000 (under cap), 0 codes dropped by price filter, all ZZ500 mid/small caps with sane pred_excess +8.51%~+11.41%.
- **Round 165 (user ACK picks, advisor packaged Step D, 2026-05-31)**: user approved picks; Step D queued (cron plist + freeze flag clean + ECS PowerShell + decision_log entry).
- **Round 166 / Phase 3 launch (2026-05-31)**: OOS Arm B activates. Real money unfrozen for OOS bucket only, hard-capped at ¥20,000/day via `ArmBBudgetTracker`. 14:30 ECS ScheduledTask Enable scheduled via `deploy/ecs/enable_intraday_pipeline.ps1` (run by user in RDP). Mac launchd monitor + monthly report live via `deploy/launchd/com.moneyprinter.arm_b_{monitor,report}.plist` (loaded by advisor on Mac side).
  - **Scope of unfreeze**: OOS Arm B only. 9:25 EOD path remains self-governing (BASELINE n2c label).
  - **Active guardrails** (round 162): (a) ≤¥20,000/day cap, (b) ADV≥1亿 + price≤¥50, (c) monthly OOS vs EOD report, (d) -5pp cumulative hard stop with auto-freeze + `Disable-ScheduledTask` alert.
  - **Red-line SOP**: hard stop ≤-5pp → automatic freeze + alert (user explicit approval required to unfreeze + re-Enable); warn ≤-3pp yellow card → human review; fill slippage > 20bps median or task failures > 2 → pause + report (manual, advisor weekly audit catches).
  - **Recovery path**: Mac side `mp.risk.freeze.unfreeze(by=..., approval_token=...)`; ECS side `deploy/ecs/enable_intraday_pipeline.ps1` re-Enables.
