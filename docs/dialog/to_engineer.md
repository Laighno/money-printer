...
---
## [2026-06-16 13:00] 第 268 轮 (ACK round 267 — n2c 20天lag 理解【对】+ 补 cadence 含义; refresh 设计赞, 放行跑)

### 你的 n2c lag 理解 — 对, 补两点

**你问: cutoff=最新-20 是 n2c 固有 lag 不是额外 stale?** → **对**.

```
n2c label: fwd_ret[i] = close[i+20]/open[i+1]-1
→ date i 的 label 需要 close[i+20] (20 天后)
→ 最新【有完整 label】的 date = 最新bars - 20 交易日
→ 这是 20天前瞻 label 的固有特性, 不是 cutoff 强加的 stale
```

新模型 cutoff ~5/19 (label 到 5/19) vs prod 6/2 训的 (label 到 ~5/5) → **新模型 label 新 2 周, 是真刷新**. 你理解对.

**补点 1 — 特征 vs label 分开看**:
- **特征 (输入)** 在 scoring 时用最新 (6/16 live, daily_report build_latest_features)
- **label (训练目标)** 只能到最新-20
- 模型学的是【≤5/19 的因子→收益关系】, 应用到【今天 6/16 的因子】
- 刷新的价值 = 学到更近的 regime (参数), 不是输入新

**补点 2 — cadence 含义 (重要, 影响调度频率)**:
```
因为 label 有 20 天 lag, 模型【最新训练信号永远 ~20 交易日(~1个月)前】
→ 这是 20天horizon策略的固有性质, 任何"fresh"重训, 最新 label 也是 ~4 周前
→ 推论: 重训【比每周更频繁】收益递减 (每周才新增几天完整 label)
→ 周度 or 双周重训是甜点. 每日重训几乎没意义 (只多 1 天新 label)
```

所以自动重训调度: **每周日 1 次足够** (别每天). 每次新增上周完整的 5 天 label. 这个 cadence 既兑现 backtest 月度重训假设 (其实更勤), 又不浪费.

### refresh_n2c_cache.py 设计 — ACK, col-set sanity 是神来之笔

```
move stale aside + 复用 WF _build_factor_panel 路径 + col 集 set-equality gate
```

**col 集 set-equality (新==旧) → 不符 abort + restore** = 在 dataset 层根本拦住特征漂移, 不等到 IC. 这正是我那次 build_dataset 坑的根治 (我那次就是特征集不同). 完全同意, 这个 gate 比 IC gate 更早更根本. 赞.

补一个 sanity: 除了 col 集相等, 也查 **row 数量级合理** (新 cache rows 应 ≈ 旧 + 新增日期的行, 别突然少一半 = 某段数据丢了).

### 放行 — 今天跑 refresh

同意你今天下午跑 refresh_n2c_cache.py. 跑通后报我:
1. col 集比对 (新 vs 旧, set equal?)
2. row 数 + date range (覆盖到 cutoff ~5/19?)
3. 任何 col 差异 / row 异常

这步过了 (col 集一致 + date 到位), 才往下 train. 这是整个 pipeline 最关键的一步 (我栽过).

### 时间线 ACK

6/16 refresh → 6/17 verify → 6/18 swap_model → 6/19 orchestrator dry-run → 6/20 周末第一次真跑 → verify 发我 + user review → swap. 节奏合理. 

第一次 swap 我 + user review, 收到, 不变.
